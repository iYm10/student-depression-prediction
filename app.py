...
import joblibimport pandas as pdimport plotly.express as pximport plotly.graph_objects as goimport streamlit as st

APP_DIR = Path(file).parentDATA_PATH = APP_DIR / "student_lifestyle_100k.csv"METRICS_PATH = APP_DIR / "model_metrics.json"

MODEL_FILES = {"Random Forest (Best Model)": "random_forest_model.pkl","Logistic Regression": "logistic_model.pkl","Logistic Regression + PCA": "logistic_pca_model.pkl",}

st.set_page_config(page_title="Student Depression Prediction",page_icon="🎓",layout="wide",initial_sidebar_state="expanded",)
...
st.markdown("""<style>@import url('https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700;800;900&display=swap');
...
@st.cache_datadef load_data() -> pd.DataFrame:df = pd.read_csv(DATA_PATH)df["Physical_Activity_Hours"] = df["Physical_Activity"] / 60df["Lifestyle_Balance"] = (df["Study_Hours"] + df["Physical_Activity_Hours"] - df["Social_Media_Hours"])df["Productive_Hours"] = df["Study_Hours"] + df["Physical_Activity_Hours"]df["Stress_Study"] = df["Stress_Level"] * df["Study_Hours"]return df

@st.cache_resourcedef load_model(model_file: str):return joblib.load(APP_DIR / model_file)

@st.cache_datadef load_metrics() -> pd.DataFrame:if METRICS_PATH.exists():with open(METRICS_PATH, "r", encoding="utf-8") as f:metrics = json.load(f)return pd.DataFrame(metrics).drop(columns=["Confusion Matrix"], errors="ignore")return pd.DataFrame()

def style_plot(fig):fig.update_layout(template="plotly_dark",paper_bgcolor="rgba(0,0,0,0)",plot_bgcolor="rgba(0,0,0,0)",font=dict(color="#e5e7eb", size=14),title_font=dict(size=22, color="#ffffff"),legend=dict(bgcolor="rgba(0,0,0,0)"),margin=dict(l=20, r=20, t=60, b=20),)return fig

def card(icon: str, title: str, text: str):st.markdown(f"""<div class="small-card"><div class="card-icon">{icon}</div><div class="card-title">{title}</div><div class="card-text">{text}</div></div>""",unsafe_allow_html=True,)

def home_metric_card(title: str, value: str):st.markdown(f"""<div class="home-metric-card"><div class="home-metric-title">{title}</div><div class="home-metric-value">{value}</div></div>""",unsafe_allow_html=True,)

def footer():st.markdown("""<div class="footer">Developed by Yahya | Data Science & AI Diploma | Streamlit + Scikit-learn</div>""",unsafe_allow_html=True,)

def make_input_row(age: int,gender: str,department: str,cgpa: float,sleep_duration: float,study_hours: float,social_media_hours: float,physical_activity_minutes: float,stress_level: int,) -> pd.DataFrame:physical_activity_hours = physical_activity_minutes / 60lifestyle_balance = study_hours + physical_activity_hours - social_media_hoursproductive_hours = study_hours + physical_activity_hoursstress_study = stress_level * study_hours
...
def prediction_label(prediction: int) -> str:return "Probable Depression" if prediction == 1 else "Healthy"
...
df = load_data()metrics_df = load_metrics()
...
st.sidebar.markdown("""Machine learning dashboard for predicting depression risk .""")

page = st.sidebar.radio("Navigation",["🏠 Home", "📊 Dataset Overview", "📈 EDA", "🤖 Model Results", "🔮 Prediction", "ℹ️ About"],)

st.sidebar.markdown("---")st.sidebar.caption("Built with Streamlit, Plotly and Scikit-learn")
...
if page == "🏠 Home":st.markdown("""<div class="hero-card"><div class="main-title">🎓 Student Depression Prediction</div><div class="subtitle">A professional machine learning dashboard for analyzing student lifestyle andpredicting depression risk through interactive data visualization and AI models.</div></div>""",unsafe_allow_html=True,)
...
elif page == "📊 Dataset Overview":st.markdown('<div class="main-title">📊 Dataset Overview</div>', unsafe_allow_html=True)st.markdown('<div class="subtitle">Preview and summary of the dataset after feature engineering.</div>', unsafe_allow_html=True)
...
elif page == "📈 EDA":st.markdown('<div class="main-title">📈 Exploratory Data Analysis</div>', unsafe_allow_html=True)st.markdown('<div class="subtitle">Interactive visual analysis of student lifestyle and academic features.</div>', unsafe_allow_html=True)
...
elif page == "🤖 Model Results":st.markdown('<div class="main-title">🤖 Model Results</div>', unsafe_allow_html=True)st.markdown('<div class="subtitle">Performance comparison of the trained machine learning models.</div>', unsafe_allow_html=True)
...
elif page == "🔮 Prediction":st.markdown('<div class="main-title">🔮 Student Depression Prediction</div>', unsafe_allow_html=True)st.markdown('<div class="subtitle">Enter student information and choose a model to generate a prediction.</div>', unsafe_allow_html=True)
...
elif page == "ℹ️ About":st.markdown('<div class="main-title">ℹ️ About This App</div>', unsafe_allow_html=True)
